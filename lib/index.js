...
const url = process.env.AUTHENTICATION_URL || 'ec2-35-168-17-78.compute-1.amazonaws.com';
...
const url$1 = process.env.ROOMS_URL || 'ec2-35-168-17-78.compute-1.amazonaws.com';
        ...
        (response) => {
          return generalRequest(`${chatURL}/${response.idRoom}/`, 'DELETE').then(
			...
			exitRoom: (_, { roomDelete }) =>
				generalRequest(`${URL$1}/${roomDelete.idRoom}`, 'DELETE', roomDelete),
	},
	Subscription: {
		roomAdded: {
...
// test route
router.get('/graphiql', apolloServerKoa.graphiqlKoa({ endpointURL: '/graphql', subscriptionsEndpoint: `ws://ec2-35-168-17-78.compute-1.amazonaws.com:5000/subscriptions` }));
